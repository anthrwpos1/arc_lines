...
struct path_point : public QPoint, public selectable
{
    ...
    double _arc_angle = M_PI / 4;
    std::complex<double> _lines_rot;
};
...
class label_for_paint : public QLabel
{
    ...
    void paintArc(QPainter &painter, const path_point& from, const path_point& to, const int pnum = 36);
    void paintArcs(QPainter &painter);
    void calc_lines_angles();
    void calcArcAngles();
    void calcClosedAngles();
public:
    bool closed = false;
    bool circled = false;
